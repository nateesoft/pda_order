...
function alertInvalid() {
    alert("สินค้าที่ท่านเลือกไม่สามารถสั่งได้จากระบบ Take Order\n กรุณาสั่งอาหารรายการนี้ที่ Cashier \n เนื่องจากเป็นสินค้าที่ต้องมีการกำหนดราคา");
}
...
function holdTableTest() {
    ...
    $.get("PDAHoldTable", function(responseJson) {
        if (responseJson !== null) {
...
function saveProduct(PCode) {
    ...
    var r = confirm("ต้องการสั่งสินค้ารายการนี้ใช่หรือไม่ ?");
    if (r === true) {
        document.getElementById("openTable").style.display = "none";

        $.get("PDASaveOrderServlet?PCode=" + PCode.id, function(responseJson) {

            if (responseJson !== null) {
                //alert("Add Item Already.");
                $.each(responseJson, function(key, value) {
                    LoadTableBill();
...
function loadTableBill() {
	var tableNo = document.getElementById("txtTableNo").value;
	...
	if(tableNo===""){
		alert("กรุณาระบุโต๊ะที่ท่านจะสั่งอาหาร!!!");
		document.getElementById("txtTableNo").focus();
		return ;
	}else{
			...
			if (responseJson !== null) {
				$.each(responseJson, function(key, value) {
					if(value[0]==="OK"){
						...
						$.get("PDATableBillServlet?table=" + table + "&cust=" + customer + "&type=" + type, function(responseJson) {
							if (responseJson !== null) {
								var inData = "";
								$.each(responseJson, function(key, value) {
									inData = "Y";
								...
								if (inData === "") {
									LoadTableBill();
									document.getElementById("tableNo").innerHTML = tableNo;
								}
...
function showVoidMsg() {
    alert("สินค้ารายการนี้ได้ถูกยกเลิกการสั่งไปแล้ว");
}
...
function addProductNew() {
    ...
    if (r === true) {
        ...
        $.get("PDASaveOrderServlet?PCode=" + selQty + "*" + PCode, function(responseJson) {
...
function LoadTableBill() {
    ...
    $.get("PDALoadTableBillServlet", function(responseJson) {
        var bgColor = "";
